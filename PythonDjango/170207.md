...
`Shape` 라는 부모 클래스는 추상 클래스 이다. 이 클래스에서는 추상 클래스인 `drawShape()` 메소드를 가지고 있다. 이 메소드 에서는 `@abstractmethod`라는 데코레이터에 의해서 추상 메소드임을 정의 한다. `@`라는 문자로 시작함에 있어 자바의 어노테이션이 생각 나지만 완전히 서로 다르다. 데코레이터에 대해서는 나중에 정리 해 보도록 하자. 
